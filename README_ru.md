...
- **`bash` и `pgrep`** в PATH — их использует helper-скрипт. Есть по умолчанию в **macOS и стандартных Linux**.
  **Windows: используйте WSL2** (обычное Linux-окружение) — это поддерживаемый путь. Нативный Windows не поддержан:
  под Git Bash `pgrep` обычно отсутствует, поэтому helper честно падает (fail-closed), а без Git Bash bash вообще
  не запустит скрипт.
