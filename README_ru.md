...
- **`bash` и `pgrep`** в PATH — их использует helper-скрипт. Есть по умолчанию в **macOS и стандартных Linux**.
  **Windows: используйте WSL2** (обычное Linux-окружение). Нативный Claude Code на Windows запускает скрипт через
  Git Bash, где `pgrep` нет — тогда helper честно падает с понятной ошибкой, а не работает без guard'а.
